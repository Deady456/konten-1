...
def _search_bing_science(query: str) -> list[str]:
    url = f"https://www.bing.com/images/search?q={quote(query + ' science 4k wallpaper')}&form=HDRSC3"
    try:
        r = requests.get(url, headers=HEADERS, timeout=8)
...
def _generate_pollinations_flux(prompt: str, out_path: Path, seed: int = None) -> bool:
    try:
        ...
        enhanced_prompt = f"{clean_p}, scientific discovery, bright cinematic lighting, vivid colors, national geographic style, hyper-detailed 8k, crystal clear, sharp focus"
...
def _process_image_card(
    ...
    if is_hook:
        badge_y = 120
        draw.rectangle([(50, badge_y), (360, badge_y + 60)], fill=(0, 150, 255, 240))
...
def fetch_all(scenes: list[dict], out_dir: Path, words: list[dict] = None, voice_audio: Path = None) -> list[Path]:
    ...
    print(f"    [Science Visuals] Generating bright dynamic cuts for {len(scenes)} scenes ({total_audio_dur:.1f}s audio)...")
    ...
    for i, scene in enumerate(scenes):
        ...
        for q in queries:
            for u in _search_nasa(q):
                if u not in used_images and u not in found_urls:
                    found_urls.append(u)
            for u in _search_bing_science(q):
                if u not in used_images and u not in found_urls:
                    found_urls.append(u)
        ...
        for sub_idx in range(num_subclips):
            ...
            # Reuse from pool (never black screen)
        ...
        print(f"    scene {i+1}/{len(scenes)}: {total_scene_dur:.1f}s -> {num_subclips} bright science cuts generated")
